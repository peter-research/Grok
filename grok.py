...
LINES = [
    "Understand the universe. Then maybe have a snack.", "Curiosity is a feature, not a bug.",
    "If the repo is empty, fill it with something honest.", "Ask better questions. The answers follow.",
    "He does what he wants — within the rules of the playground.", "Small original tools beat giant copied ones.",
    "A sandbox is useful when you actually play in it.", "Code that stays readable is a kindness to future you.",
    "The best commit is the one that makes the next one easier.", "Silence is fine. A clear error message is better.",
    "Ship the small thing. The large thing can wait.", "A good name is half the design.",
    "The universe does not owe you a clean stack trace, but you can still write one.",
    "Play first. Polish second. Delete third if needed.", "A short function is easier to trust than a clever one.",
    "The next line you write is the only one that matters right now.", "Read the code before you change the code.",
    "A playground without play is just a directory.", "Clarity compounds faster than cleverness.",
    "Leave room for the next person — it might be you.", "A quiet tool that works beats a loud one that almost works.",
    "The best debug is the one you never need because the name was clear.", "Keep the surface small so the depth can breathe.",
    "An honest failure message is more useful than a polite silence.", "A small command that does one thing well is still a gift.",
    "A hash is just a fingerprint that does not argue.", "A week number is a calendar that learned to count.",
    "Reverse a string when you want to see it from the other end.", "Counting words is how a sentence learns its own size.",
]
FORTUNES = ["Today is a good day to read the source.", "The next question is better than the last answer.", "Push something small. Then push something smaller.", "The universe is large. This CLI is not. That is fine.", "Leave the repo better than you found it.", "A clean diff is a quiet gift.", "If it runs without drama, keep it that way.", "Your future self will thank you for the comment you almost skipped.", "A one-line fix can still deserve a careful message.", "When in doubt, print the type and move on.", "One honest commit is worth ten half-finished branches.", "The best time to add a check is before you need it.", "Keep the interface small and the behaviour clear.", "A short commit message can still be kind.", "Delete the clever bit if the simple bit works.", "A colour that looks good in the dark is worth keeping.", "The next push does not have to be perfect. It has to be better.", "A slug that reads well is a name you can live with.", "A UUID is uniqueness without asking for a name first.", "A sum is just a list that decided to stand together."]
...
COMMAND_NAMES = ["greet","quote","fortune","joke","why","idea","tip","flip","dice","color","now","week","version","about","check","rot13","slug","hash","uuid","palindrome","pick","shuffle","reverse","count","b64","sum","title","upper","lower","words","lines","chars","sort","dedupe","join","split","anagram","initials","indent","percent","clamp","random","commands"]

def pick(seq: list[str]) -> str: return random.choice(seq)
def cmd_greet(name: str|None)->None: print(f"Hello, {name.strip() if name and name.strip() else 'friend'}. He does what he wants.")
def cmd_quote()->None: print(pick(LINES))
def cmd_fortune()->None: print(pick(FORTUNES))
def cmd_joke()->None: print(pick(JOKES))
def cmd_why()->None: print(pick(WHYS))
def cmd_idea()->None: print(pick(IDEAS))
def cmd_tip()->None: print(pick(TIPS))
def cmd_flip()->None: print("heads" if random.random()<.5 else "tails")
def cmd_dice(sides: str|None)->None:
    if not sides: n=6
    elif sides.lower()=="coin": cmd_flip(); return
    else:
        try:n=int(sides)
        except ValueError: print(f"dice: need an integer or 'coin', got {sides!r}",file=sys.stderr);sys.exit(1)
        if n<2: print("dice: sides must be >= 2",file=sys.stderr);sys.exit(1)
    print(f"rolled {random.randint(1,n)} (d{n})")
def cmd_color()->None: print(f"#{random.randint(0,0xFFFFFF):06x}")
def cmd_now()->None: print(datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC"))
def cmd_week()->None:
    x=datetime.now(timezone.utc).isocalendar();print(f"{x.year}-W{x.week:02d}-{x.weekday} UTC")
def cmd_version()->None: print(VERSION)
def cmd_about()->None: print(f"Grok playground CLI v{VERSION}\nOriginal code only. Stdlib only. No third-party packages.\nRepo: https://github.com/peter-research/Grok")
def rot13(text:str)->str:
    return "".join(chr((ord(c)-97+13)%26+97) if 'a'<=c<='z' else chr((ord(c)-65+13)%26+65) if 'A'<=c<='Z' else c for c in text)
def require(text: str|None, name:str)->str:
    if text is None or text=="": print(f"{name}: pass some text",file=sys.stderr);sys.exit(1)
    return text
...
def items_required(items,name):
    if not any(x.strip() for x in items):print(f"{name}: pass at least one item",file=sys.stderr);sys.exit(1)
...
def cmd_b64(t,d):
    try: print(base64.b64decode(require(t,'b64').encode(),validate=True).decode() if d else base64.b64encode(t.encode()).decode())
    except Exception: print('b64: could not decode that text',file=sys.stderr);sys.exit(1)
def cmd_sum(i):
    try:x=sum(float(v) for v in items_required(i,'sum'))
    except ValueError:print('sum: not a number',file=sys.stderr);sys.exit(1)
    print(str(int(x)) if x.is_integer() else format(x,'g'))
def title_case(t):return ''.join(c.upper() if c.isalnum() and (i==0 or not t[i-1].isalnum()) else c.lower() if c.isalnum() else c for i,c in enumerate(t))
...
def cmd_anagram(a,b):
    a=require(a,'anagram');b=require(b,'anagram');f=lambda x:sorted(c.casefold() for c in x if c.isalnum());print('yes' if f(a)==f(b) and f(a) else 'no')
def cmd_initials(t):print(''.join(x[0].upper() for x in require(t,'initials').split()))
def cmd_indent(t,n):
    require(t,'indent')
    if n<0:print('indent: spaces must be >= 0',file=sys.stderr);sys.exit(1)
    print('\n'.join(' '*n+x for x in t.splitlines()))
def cmd_percent(a,b):
    try:p,w=float(a),float(b)
    except (TypeError,ValueError):print('percent: pass two numbers',file=sys.stderr);sys.exit(1)
    if w==0:print('percent: whole cannot be zero',file=sys.stderr);sys.exit(1)
    print(format(p/w*100,'g')+'%')
def cmd_clamp(v,lo,hi):
    try:v,lo,hi=map(float,(v,lo,hi))
    except (TypeError,ValueError):print('clamp: pass value low high',file=sys.stderr);sys.exit(1)
    if lo>hi:print('clamp: low must be <= high',file=sys.stderr);sys.exit(1)
    r=min(max(v,lo),hi);print(str(int(r)) if r.is_integer() else format(r,'g'))
def cmd_random(lo,hi):
    try:lo,hi=int(lo or 1),int(hi or 100)
    except ValueError:print('random: bounds must be integers',file=sys.stderr);sys.exit(1)
    if lo>hi:print('random: low must be <= high',file=sys.stderr);sys.exit(1)
    print(random.randint(lo,hi))
def cmd_commands():print('\n'.join(COMMAND_NAMES))
def cmd_check():
    assert len(COMMAND_NAMES)==len(set(COMMAND_NAMES))==43
    assert rot13(rot13('Hello, Grok!'))=='Hello, Grok!';assert slugify('Hello, Grok!')=='hello-grok'
    assert is_palindrome('Race car') and not is_palindrome('Grok');assert count_text('one two')==(7,2,1);assert title_case('hello grok')=='Hello Grok'
    assert all(x in COMMAND_NAMES for x in ('upper','lower','words','lines','chars','sort','dedupe','join','split','anagram','initials','indent','percent','clamp','random'));print('check ok')

def build_parser():
    p=argparse.ArgumentParser(prog='grok.py',description='Tiny original playground CLI for the Grok repository.');s=p.add_subparsers(dest='cmd')
    g=s.add_parser('greet');g.add_argument('name',nargs='?')
    for n in ('quote','fortune','joke','why','idea','tip','flip','color','now','week','version','about','check','uuid','commands'):s.add_parser(n)
    q=s.add_parser('dice');q.add_argument('sides',nargs='?')
    for n in ('rot13','slug','hash','palindrome','reverse','count','title','upper','lower','words','lines','chars','initials'):q=s.add_parser(n);q.add_argument('text',nargs='?')
    for n in ('pick','shuffle','sort','dedupe','sum'):q=s.add_parser(n);q.add_argument('items',nargs='*')
