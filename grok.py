...
def run_length(text):
    ...
    for c in text[1:]:
        if c == prev: n += 1
            ...
            out.append(f'{n}{prev}' if n > 1 else prev); prev=c; n=1
...
def text_box(text):
    ...
    return '\n'.join([top, *body, top])
def to_roman(n):
    if n < 1 or n > 3999: raise ValueError('range')
...
def cmd_roman(n):
    ...
    try: print(to_roman(v))
        ...
        print('roman: need an integer from 1 to 3999', file=sys.stderr); sys.exit(1)
def cmd_entropy(t):
    print(format(shannon_entropy(require(t,'entropy')),'g'))
def from_roman(text):
    ...
    for c in reversed(raw):
        if c not in values: raise ValueError('bad')
        v = values[c]
        if v < prev: total -= v
        else:
            total += v; prev = v
    if total < 1 or total > 3999 or to_roman(total) != raw: raise ValueError('bad')
    return total
def is_isogram(text):
...
def to_tap(text):
    ...
    for c in text.casefold():
        if c == 'z': c = 'x'
        if c in TAP:
            i = TAP.index(c); out.append(f'{i//5+1}{i%5+1}')
        elif c.isspace(): out.append('/')
    return ' '.join(out) if out else '.'
def cmd_unroman(t):
    try: print(from_roman(require(t,'unroman')))
    except ValueError:
...
def cmd_tap(t): print(to_tap(require(t,'tap')))
def cmd_fib(n):
    raw = require(n, 'fib')
    try: v = int(raw)
    except ValueError:
...
def cmd_prime(n):
    ...
    try: v = int(raw)
        ...
        print('prime: need an integer', file=sys.stderr); sys.exit(1)
    print('yes' if is_prime(v) else 'no')
def cmd_gcd(a, b):
    try: x, y = int(require(a, 'gcd')), int(require(b, 'gcd'))
    except ValueError:
        print('gcd: pass two integers', file=sys.stderr); sys.exit(1)
    print(gcd_int(x, y))
def cmd_lcm(a, b):
    try: x, y = int(require(a, 'lcm')), int(require(b, 'lcm'))
    except ValueError:
        print('lcm: pass two integers', file=sys.stderr); sys.exit(1)
    print(lcm_int(x, y))
def cmd_unrle(t):
    try: print(undo_rle(require(t, 'unrle')))
    except ValueError:
        print('unrle: could not decode that run-length text', file=sys.stderr); sys.exit(1)
def cmd_check():
    assert len(COMMAND_NAMES)==len(set(COMMAND_NAMES))==76
    ...
    assert all(x in COMMAND_NAMES for x in ('day','echo','wrap','sample','repeat','morse','unique','yesno','leet','vowels','bin','hex','caesar','consonants','pig','ascii','snake','camel','nato','freq','entropy','lev','rle','box','roman','unroman','isogram','tap','fib','prime','gcd','lcm','unrle'))
...
def build_parser():
    ...
    for n in ('rot13','slug','hash','palindrome','reverse','count','title','upper','lower','words','lines','chars','initials','echo','morse','unique','leet','vowels','bin','hex','consonants','pig','ascii','snake','camel','nato','freq','entropy','rle','box','roman','unroman','isogram','tap','fib','prime','unrle'):
