...
def cmd_entropy(t):
    ...
    print(format(e,'g'))
def cmd_check():
    assert len(COMMAND_NAMES)==len(set(COMMAND_NAMES))==68
    ...
    assert all(x in COMMAND_NAMES for x in ('day','echo','wrap','sample','repeat','morse','unique','yesno','leet','vowels','bin','hex','caesar','consonants','pig','ascii','snake','camel','nato','freq','entropy','lev','rle','box','roman'))
...
def build_parser():
    ...
    for n in ('rot13','slug','hash','palindrome','reverse','count','title','upper','lower','words','lines','chars','initials','echo','morse','unique','leet','vowels','bin','hex','consonants','pig','ascii','snake','camel','nato','freq','entropy','rle','box','roman'):
