...
def mode_nums(vals):
    ...
    modes = [k for k, n in counts.items() if n == best]
...
def pow_nums(base, exp):
    if abs(exp) > 12 or abs(base) > 1_000_000:
        raise ValueError("range")
    return base ** exp
...
def cmd_check():
    assert len(COMMAND_NAMES) == len(set(COMMAND_NAMES)) == 86
    ...
    assert mode_nums([1, 2, 2, 3]) == 2
    assert abs(stddev_nums([2, 4, 4, 4, 5, 5, 7, 9]) - 2) < 1e-9
    assert pow_nums(2, 10) == 1024
    assert unb64(base64.b64encode(b"Grok").decode()) == "Grok"
...
def build_parser():
    ...
    for n in (
        ...
        "revwords", "factors", "wrap", "caesar", "unb64",
    ):
        q = s.add_parser(n)
    ...
    for n in (
        ...
        "sort", "dedupe", "join", "split", "unique", "percent", "clamp", "random", "lev", "mode", "stddev", "pow", "hamming",
    ):
        q = s.add_parser(n)
...
def main(argv=None):
    ...
    h = {
        ...
        "mode": lambda: print(" ".join(fmt_num(x) for x in (lambda m: m if isinstance(m, list) else [m])(mode_nums([float(x) for x in items()])))),
        "stddev": lambda: print(fmt_num(stddev_nums([float(x) for x in items()]))),
        "pow": lambda: print(fmt_num(pow_nums(float(items()[0]), float(items()[1])))),
        "unb64": lambda: print(unb64(text())),
        "hamming": lambda: print(hamming(items()[0], items()[1])),
    }
    try:
        h[a.cmd]()
