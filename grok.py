...
def pig_word(word):
    ...
    first=letters[0]
    rest=''.join(letters[1:])
    tail='yay' if first.casefold() in 'aeiouy' else 'ay'
    moved=rest+first+tail
    if word[0].isupper():
...
def cmd_check():
    assert len(COMMAND_NAMES)==len(set(COMMAND_NAMES))==59
    ...
    assert all(x in COMMAND_NAMES for x in ('day','echo','wrap','sample','repeat','morse','unique','yesno','leet','vowels','bin','hex','caesar','consonants','pig','ascii'))
    print('check ok')
def build_parser():
    ...
    for n in ('rot13','slug','hash','palindrome','reverse','count','title','upper','lower','words','lines','chars','initials','echo','morse','unique','leet','vowels','bin','hex','consonants','pig','ascii'):
