...
def cmd_check():
    assert len(COMMAND_NAMES)==len(set(COMMAND_NAMES))==51
    ...
    assert all(x in COMMAND_NAMES for x in ('day','echo','wrap','sample','repeat','morse','unique','yesno'))
    print('check ok')
def build_parser():
    ...
    for n in ('rot13','slug','hash','palindrome','reverse','count','title','upper','lower','words','lines','chars','initials','echo','morse','unique'):
