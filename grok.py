...
COMMAND_NAMES = ["greet","quote","fortune","joke","why","idea","tip","flip","dice","color","now","week","day","echo","version","about","check","rot13","slug","hash","uuid","palindrome","pick","sample","shuffle","reverse","count","wrap","b64","sum","title","upper","lower","words","lines","chars","sort","dedupe","join","split","anagram","initials","indent","percent","clamp","random","commands"]
...
def cmd_check():
    assert len(COMMAND_NAMES)==len(set(COMMAND_NAMES))==47
    ...
    assert all(x in COMMAND_NAMES for x in ('day','echo','wrap','sample','upper','lower','words','lines','chars','sort','dedupe','join','split','anagram','initials','indent','percent','clamp','random'))
    print('check ok')
def build_parser():
    ...
    for n in ('quote','fortune','joke','why','idea','tip','flip','color','now','week','day','version','about','check','uuid','commands'): s.add_parser(n)
    q=s.add_parser('dice'); q.add_argument('sides', nargs='?')
    for n in ('rot13','slug','hash','palindrome','reverse','count','title','upper','lower','words','lines','chars','initials','echo'):
