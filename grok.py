...
LINES = [
    ...
    "Reverse a string when you want to see it from the other end.",
    "Counting words is how a sentence learns its own size.",
]
...
FORTUNES = [
    ...
    "A sum is just a list that decided to stand together.",
]
...
JOKES = [
    ...
    "Base64 walked into a party and said: just put me on the table, I'll wrap myself.",
]
...
WHYS = [
    ...
    "Because a title case line still wants to stand up straight.",
]
...
IDEAS = [
    ...
    "Add a reverse command so text can look over its shoulder.",
]
...
TIPS = [
    ...
    "Count before you claim the sentence is short.",
]
...
def cmd_reverse(text: str | None) -> None:
    if text is None:
        print("reverse: pass some text", file=sys.stderr)
        sys.exit(1)
    print(text[::-1])
...
def cmd_count(text: str | None) -> None:
    if text is None:
        print("count: pass some text", file=sys.stderr)
        sys.exit(1)
    chars, words, lines = count_text(text)
    print(f"{chars} chars · {words} words · {lines} lines")


def cmd_b64(text: str | None, decode: bool) -> None:
    if text is None or text == "":
        print("b64: pass some text", file=sys.stderr)
        sys.exit(1)
    try:
        if decode:
        ...
        print("b64: could not decode that text", file=sys.stderr)
        sys.exit(1)


def cmd_sum(items: list[str]) -> None:
    if not items:
        print("sum: pass one or more numbers", file=sys.stderr)
        sys.exit(1)
    total = 0.0
    for item in items:
        try:
            ...
            print(f"sum: not a number: {item!r}", file=sys.stderr)
            sys.exit(1)
    if total.is_integer():
        print(str(int(total)))
    else:
        print(format(total, "g"))
...
def title_case(text: str) -> str:
    ...
    for ch in text:
        if ch.isalnum():
            parts.append(ch.upper() if start else ch.lower())
            start = False
        else:
            parts.append(ch)
            start = True
...
def cmd_title(text: str | None) -> None:
    if text is None or text == "":
        print("title: pass some text", file=sys.stderr)
        sys.exit(1)
...
def cmd_check() -> None:
    ...
    encoded = base64.b64encode(b"Grok").decode("ascii")
    assert base64.b64decode(encoded) == b"Grok"
    assert len(COMMAND_NAMES) == len(set(COMMAND_NAMES))
    for name in ("reverse", "count", "b64", "sum", "title"):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    rv = sub.add_parser("reverse", help="reverse the given text")
    rv.add_argument("text", nargs="?", default=None)

    ct = sub.add_parser("count", help="count chars, words and lines")
    ct.add_argument("text", nargs="?", default=None)

    b = sub.add_parser("b64", help="base64 encode or decode text")
    b.add_argument("text", nargs="?", default=None)
    b.add_argument("--decode", action="store_true", help="decode instead of encode")

    sm = sub.add_parser("sum", help="add numbers")
    sm.add_argument("items", nargs="*")

    tt = sub.add_parser("title", help="title-case the given text")
    tt.add_argument("text", nargs="?", default=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    handlers = {
        ...
        "reverse": lambda: cmd_reverse(args.text),
        "count": lambda: cmd_count(args.text),
        "b64": lambda: cmd_b64(args.text, args.decode),
        "sum": lambda: cmd_sum(args.items),
        "title": lambda: cmd_title(args.text),
